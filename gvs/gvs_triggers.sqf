...
- Generates Event handlers for display of the trigger locations when a player
  is within a vehicle
...
The trigger variables can be adjusted, but keep in mind that sounds are fixed
in duration, so adjusting a a setting like _veh_per_fuel_time to some value
other than 2 seconds will throw off the sound loop. Sounds for ammo or repair
ar not so much affected but times that are too low, may cause sounds to play
