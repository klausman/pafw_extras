/*
  ...
  B - Added parameter processing function and string searching function based
      off of work by Xeno

  Features:
  - Allows other scripts to use friendly calls for information lookups to
    vehicles, weapons, and magazines
...
String_Search = {
    private _source = toArray (_this select 0);
    private _find = toArray (_this select 1);
    
    private _sourceCount = count _source;
    private _findCount = count _find;
    private _foundPos = -1;
    private _initialIndex = 0;
    if ((_sourceCount - _initialIndex) < _findCount) exitWith {_foundPos};
    
    private _findIndex = 0;
