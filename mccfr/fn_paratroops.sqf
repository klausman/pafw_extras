// Control the paratroop reinforcement spawn
...
switch (_paraSide) do {
    case "WEST": { _p_mcc_spawnfaction = "BLU_F"; };
    case "EAST": { _p_mcc_spawnfaction = "OPF_F"; };
    case "GUE":  { _p_mcc_spawnfaction = "IND_F"; };
    default      { _p_mcc_spawnfaction = "OPF_F"; };
};
...
{
    if ( ((_x select 3) == "Rifle Squad")  ) then { 
...
if ( _paraMode == 2 ) then {
    heliPad setposATL [_dropPos select 0,_dropPos select 1,(_dropPos select 2) + 25];
};
...
//trigger later when parachuting
if ( _paraMode > 0 ) then {
    waitUntil { sleep 1;((_heli distance _dropPos) < ((getPosATL _heli select 2) + 100)) || !(canMove _heli)}; 
} else {
    waitUntil { sleep 1;((_heli distance _dropPos) < ((getPosATL _heli select 2) + 30)) || !(canMove _heli)};i
};
