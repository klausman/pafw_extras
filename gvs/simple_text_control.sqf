...
B - Altered parameters and order for stc_DisplayShow, stc_DiplayHide, and
    stc_DisplayWrite. Use of cutRsc layer now. 10/13/2013
...
Accepts parameters:
0: _layer: The cutRsc layer number (keep different controls on different layers
...
5: _x: 0 to 1 Absolute x position or safeZoneX relative x position of the
   display control (recommend using safeZoneX + 0 to 1)
6: _y: 0 to 1 Absolute y position or safeZoneY relative y position of the
   display control (recommend using safeZoneY + 0 to 1)
...
will display the dialog on the left side half way down as a square box. The box
will be blank until
...
stc_DisplayWrite = {
    ...
    if (count _this > 0) then {
        private _display_name = _this select 0;
        private _control_number = _this select 1;
