...
B - Heavily revised, runs on server only, prevents vehicles from reentry after
    service instead of preventing the player from reentry. 4/21/2013

Description:

Executed at startup. Prevents a vehicle from being reserviced for a specified
period after using a service point successfully by checking it against the
Public_Banned_Vehicle_Service_List array. The script sleeps on a 10 second
loop.    

Default wait is 200 seconds (20 passes * 10 seconds).   

It is recommended to not lower the sleep time of 10 seconds to keep the
